...
CharCmds['\\'] = P(MathCommand, function(_, super_) {
  ...
  _.createBlocks = function() {
    ...
    this.ends[L].write = function(cursor, ch) {
      ...
      if (ch.match(/[a-z]/i)) VanillaSymbol(ch).createLeftOf(cursor);
        ...
        if (ch !== '\\' || !this.isEmpty()) cursor.parent.write(cursor, ch);  // this.parent.parent.write(cursor, ch);
      }
    };
  ...
  _.renderCommand = function(cursor) {
    ...
    if (!latex) latex = ' ';
    var cmd = LatexCmds[latex] // || Environments[latex];  // Expose Environment tags as well to be used with \\
    if (cmd) {
      cmd = cmd(latex);
