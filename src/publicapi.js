...
function getInterface(v) {
  ...
  APIClasses.EditableField = P(AbstractMathQuill, function(_, super_) {
    ...
    _.focus = function(args) { 
        if(args) this.__controller.textarea[0].focus(args);  //e.g. {preventScroll: true}
    ...
    _.keystroke = function(keys, event) {
      var keys = keys.replace(/^\s+|\s+$/g, '').split(/\s+/);
      for (var i = 0; i < keys.length; i += 1) {
        this.__controller.keystroke(keys[i], event || { preventDefault: noop });
      }
      return this;
    };
